...
public abstract class NRegisterAllocator {
    ...
    protected void buildIntervals() {
        ...
        //go through blocks from down to top
        for (int i = cfg.basicBlocks.size() - 1; i >= 0; i--) {
            ...
            int blockStart = currBlock.lir.get(0).id; //25
            int blockEnd = currBlock.lir.get(currBlock.lir.size() - 1).id; //50
            BitSet liveOut = currBlock.liveOut;  //leave outs!! 37, 38
            ...
            for (int j = currBlock.lir.size() - 1; j >= 0; j--) {
                ...
                NLIRInstruction instruction = currBlock.lir.get(j);
