...
public class NLinearRegisterAllocator extends NRegisterAllocator {
    ...
    public void allocation() {
        ...
        registerPrinter("INTERVALLSSSSSS ", cfg.intervals);
//        printStatus("After building intervals");

        //sort the intervals via starting interval!!
...
//        printStatus("AFter cfg register intervals");
...
//        printStatus("After block ");

        NInterval currInterval; // the current interval
        int position; // the current interval's first start position
...
//            printStatus("At start of each iteration " +  loop++);
            currInterval = unhandled.remove(0);

//            registerPrinter("Current : ", currInterval);

            position = currInterval.firstRangeStart();

//            System.out.println("Current position " + position);

            tmp = new ArrayList<NInterval>();
            for (NInterval anActive : active) {
                if (anActive.lastNRangeStop() < position) {
                    tmp.add(anActive);
                } else if (!anActive.isLiveAt(position)) {
                    inactive.add(anActive);
                    tmp.add(anActive);
                }
            }
            for (NInterval nonActive : tmp) active.remove(nonActive);

            tmp = new ArrayList<NInterval>();
            for (NInterval anInactive : inactive) {
                if (anInactive.lastNRangeStop() < position) {
                    tmp.add(anInactive);
                } else if (anInactive.isLiveAt(position)) {
                    active.add(anInactive);
                    tmp.add(anInactive);
                }
            }

//            printStatus("After InActive loop");
//            registerPrinter("temp", tmp);

            for (NInterval nonInActive : tmp) inactive.remove(nonInActive);


            //not free register found!!!
            //temporary registers actually!!
            //i don't like this code, cause more explanation is needed!!
            if (!this.foundFreeRegFor(currInterval)) {// check
                this.allocateBlockedRegFor(currInterval); // never fails
            }

            active.add(currInterval);
//            printStatus("Bottom part of the loop" );
//            registerPrinter("temp", tmp);
        }
    ...
    private void printStatus(String s) {
        ...
        registerPrinter("Unhandled" , unhandled);
        registerPrinter("Actve", active);
        registerPrinter("Inactive", inactive);
        registerPrinter("CFG intervals" , cfg.intervals);
        System.out.println();
    }

    private void registerPrinter(String type, ArrayList<NInterval> intervals) {


        System.out.print(type + "  => ");
        for(NInterval each : intervals){
            System.out.print(String.format(" [%s, %s]", each.firstRangeStart(),
                    each.lastNRangeStop()));
        }
        System.out.println();
    }

    private void registerPrinter(String type, NInterval each) {
